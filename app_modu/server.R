function(input, output, session) {
  ...
  observeEvent(input$sub2,{
    train_param <- callModule(return_trainmap_Server, "train_map")
    param_train_es<-as.data.frame(({ train_param() }))
    write.csv(param_train_es,"C:/Users/reto.spielhofer/OneDrive - NINA/Documents/Projects/WENDY/Questionnaire/shiny_output/train_param_dat.csv",
              row.names = T)
