function(input, output, session) {
  ...
  observeEvent(input$sub3, {
  callModule(training_Server,"es_train",data_copy)
  })
...
#   df2 <- shiny::reactiveValues(types = sapply(dat, class),
#                               data = data_copy,
#                               zoom_to = zoomto,
#                               edit_logic = le)
#   
...
#       module = editMod,
#       leafmap = {
#         mapv <- mapview::mapview(df2$zoom_to,
#                                  map.types = "CartoDB.Positron")@map %>%
#           leaflet::hideGroup('df2$zoom_to') %>%
#           leafem::addFeatures(data = df2$data,
#                               layerId = df2$data$leaf_id,
#                               group = 'editLayer',
#                               popup = leafpop::popupTable(df2$data))
#         mapv},
#       id = "map2",
#       targetLayerId = 'editLayer',
#       sf = TRUE,
...
#   
#   
#   proxy_map <- leaflet::leafletProxy('map-map2', session)
#   # render new row form based on the existing data structure
# 
# #   
#   shiny::observe({
# 
#     output$dyn_form2 <- shiny::renderUI({
# 
#       shiny::tagList(
#         lapply(1:length(df2$types), function(n){
#           name <- names(df2$types[n])
#           label <- paste0(names(df2$types[n]), ' (', df2$types[n], ')')
#           if (df2$types[n] == 'character') {
#             shiny::textInput(name, label, width = '100%')
#           }  else if (df2$types[n] %in% c('numeric','integer')) {
#             shiny::sliderInput(name, label,1,5,3,1, width = '100%')
#           }
#         }),
#         # we don't want to see this element but it is needed to form data structure
...
# 
#     n <- grep('leaf_id|geom', colnames(df2$data)) # used to hide geometry/leaf_id column
# 
#     DT::datatable(
#       df2$data,
#       options = list(scrollY="200px",
...
#   })
# #   
#   proxy = DT::dataTableProxy('tbl2')
# #   
# #   # modify namespace to get map ID
#   nsm <- function(event="", id="map2") {
#     paste0(session$ns(id), "-", event)
#   }
# #   
#   EVT_DRAW <- "map_draw_new_feature"
...
# 
#             # creates first column and row (must be more elegant way)
#             new_row <- data.frame(X = input[[names(df2$types[1])]])
#             colnames(new_row) <- names(df2$types[1])
# 
#             # remaining columns will be correct size
#             for (i in 2:length(df2$types)) {
#               new_row[names(df2$types[i])] <- input[[names(df2$types[i])]]
#             }
# 
#             new_row <- sf::st_as_sf(new_row, geometry =
#                                       sf::st_sfc(sf::st_point()), crs = APP_CRS)
...
# #   
#   addRowOrDrawObserve(EVT_ADD_ROW, id = NA)
#   addRowOrDrawObserve(EVT_DRAW, id = 'map2')
# #   
#   addDrawObserve <- function(event) {
#     shiny::observeEvent(
#       input[[nsm(event)]],
...
#           }
# 
#           df2$data <- dplyr::filter(df2$data, !df2$data$leaf_id %in% ids)
#           df2$ids <- ids
# 
#         } else if (event == EVT_EDIT) {
# 
...
# 
#             if (evt_type == 'Point') {
#               sf::st_geometry(df2$data[df2$data$leaf_id %in% leaf_id,]) <- sf::st_sfc(sf::st_point(geom))
#             } else if (evt_type == 'Polygon'){
#               geom <- matrix(geom, ncol = 2, byrow = T)
#               sf::st_geometry(df2$data[df2$data$leaf_id %in% leaf_id,]) <- sf::st_sfc(sf::st_polygon(list(geom)))
#             } else if (evt_type == 'LineString'){
#               geom <- matrix(geom, ncol = 2, byrow = T)
#               sf::st_geometry(df2$data[df2$data$leaf_id %in% leaf_id,]) <- sf::st_sfc(sf::st_linestring(geom))
#             }
#           }
# 
#         } else {
...
#           # replace if draw or edit
#           if(skip==FALSE) {
#             sf::st_geometry(df2$data[selected,]) <- sf::st_geometry(
#               mapedit:::st_as_sfc.geo_list(evt))
# 
#             #adding the leaf_id when we draw or row_add
#             df2$data[selected, 'leaf_id'] <-
#               as.integer(evt$properties[['_leaflet_id']])
# 
#           }
#         }
...
#   shiny::observeEvent(input$tbl_cell_edit, {
# 
#     df2$data <- DT::editData(df2$data, input$tbl_cell_edit, 'tbl',
#                             resetPaging = F)
#     DT::replaceData(proxy, df2$data, rownames = FALSE, resetPaging = FALSE)
# 
#   })
#   
#   
#   # provide mechanism to return after all done
#   gee_poly2<-shiny::eventReactive(input$sub4, {
#     user_es <- rand_es_sel2()
#     userID <- userID()
#     out <- sf::st_sf(df$data,crs=user_crs)
#     
#     ### area and amount of polys
...
#     blog <-input$argue
#     ###
#     callModule(ESmoduleServer, "es_quest2",area,nrow(out),blog, userID, user_es$es_id)
#     # dat_quest<-as.data.frame(({ quest() }))     
#     
#     ##remove first col since this is the training dat
#     geom<-as.data.frame(out)[-1,]
...
#     # pol_path <- "C:/Users/reto.spielhofer/OneDrive - NINA/Documents/Projects/WENDY/PGIS_ES/output/train_polys_R1"
#     # st_write(geom, paste0(pol_path, "/",userID,"_",user_es$es_id, ".shp"), delete_layer = TRUE)
#     gee_poly2<-rgee::sf_as_ee(geom, via = "getInfo")
#     # callModule(gee_Server,"map_extra",gee_poly,"recr")
#     
#   })
#   
#   
#   prediction2 <- eventReactive(input$sub2, {
#     gee_poly<-gee_poly2()
#     bands <- c("landcover","be75","landcover_count","slope","slope_mean","aspect")
#     
#     poly_pts = comb$select(bands)$sampleRegions(
...
#     
#     rand_es_sel2 <- rand_es_sel2()
#     es_ak2<-rand_es_sel2$es_id
#     userID <- userID()
#     prediction2<-prediction2()
#     assetid <- paste0(ee_get_assethome(), '/rgee/individual_R1_',es_ak2,"/", userID)
#     task_img <- ee_image_to_asset(
#       image = prediction2,
#       assetId = assetid,
#       overwrite = F,
