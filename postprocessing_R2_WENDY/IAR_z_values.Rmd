...
for(a in 1:nrow(IAR_poly)){
  ...
  area_list[b]<-sum(st_area(tmp_poly))
  # and add the mean IAR of that area to the IAR list
  iar_list[b]<-mean(tmp_poly$constant.x,na.rm=T)
  #just for security
  rm(tmp_poly)

}#/loop distances
  #a df with areas and means (for all the distances) of a single cell
  IAR_df<-as.data.frame(cbind(unlist(area_list),unlist(iar_list)))
  colnames(IAR_df)<-c("area","IAR_mean")
  # the log of both
  IAR_df$logA<-log(IAR_df$area)
  IAR_df$logI<-log(IAR_df$IAR_mean)
...
plot(z_rast)
writeRaster(z_rast,paste0(out_path,"/Z_raster/Z_rast_",esID,".tif"))
...
z_path<-paste0(main_path,"/output/Z_raster/Z_rast_")
esID<-"drinking_wat"
...
## Aorig
A_orig<-as.numeric(st_area(IAR_poly[1,])/1000000)
...
PDF_LO<-IAR_poly$constant[poly_id]*(1-((A_orig-A_lost_LO)/A_orig)^z_poly$Z_rast_drinking_wat[poly_id])

##??/sum(IAR_poly$constant)
...
PDF_DS_cult<-IAR_poly$constant[PDF_LO]*(1-((A_orig-A_lost_disturbance_cult)/A_orig)^z_poly$Z_rast_drinking_wat[PDF_LO])
##/sum??
