...
#'
#' }
# editAttributes <- function(dat, zoomto = NULL, col_add = F, reset = TRUE, provider = 'CartoDB.Positron', testing = FALSE){
  ...
  editattribute_UI = function(id){
    ns <- NS(id)
    tagList(
    shinyWidgets::useSweetAlert(),
    shiny::fluidPage(
      shiny::fluidRow(
        shiny::column(12, editModUI(ns("map")))
      ),
      tags$hr(),
      shiny::fluidRow(
        shiny::column(ifelse(FALSE, 6, 9),
                      DT::dataTableOutput(ns("tbl"),width="100%", height=200)),
        shiny::column(3,
                      shiny::wellPanel(
                        shiny::h3('Add New Row'),
                        shiny::uiOutput(ns('dyn_form')),
                        shinyWidgets::actionBttn(ns("row_add"), "Row",
        ...
        {if (FALSE) {
          shiny::column(3,
                        shiny::wellPanel(
                          ...
                          shiny::textInput(ns('new_name'), 'New Column Name', width = '100%'),
                          shiny::radioButtons(ns('new_type'), 'Column Type', choices = c('character', 'numeric', 'integer', 'Date')),
                          shinyWidgets::actionBttn(ns("col_add"), "Column",
                                                   icon = shiny::icon('plus'),
                                                   style = 'material-flat',
                                                   block = TRUE,
                                                   color = 'primary',
      ...
      shiny::fluidRow(tags$hr(),
                      shiny::div(style = 'padding: 20px',
                                 shinyWidgets::actionBttn(ns("donebtn"), "Done",
  ...
  editattribute_server <- function(input, output, session,data_copy,dat,zoomto,le) {
    ns <- session$ns
    ...
    shiny::observeEvent(input$col_add, {
      ...
      if (nchar(input$new_name)==0) {
        ...
        shiny::updateTextInput(session, ns('new_name'), value = NA)
        ...
        shiny::tagList(
          lapply(1:length(df$types), function(n){
            ...
            if (df$types[n] == 'character') {
              shiny::textInput(ns(name), label, width = '100%')
            } else if (df$types[n] == 'factor') {
              shiny::selectInput(ns(name), label, width = '100%',
                                 choices = levels(dat[[names(df$types[n])]]),
                                 selected = NULL,
                                 selectize = TRUE)
            } else if (df$types[n] %in% c('numeric','integer')) {
              shiny::numericInput(ns(name), label, width = '100%', value = NA)
            } else if (df$types[n] == 'Date') {
              shiny::dateInput(ns(name), label, width = '100%', value = NA)
            }
          }),
            ...
            if(!is.null(input$tbl_rows_selected)){
              ...
              if(isTRUE(reset)){
                for (i in 1:length(df$types)) {
                  ...
                  if (typ == 'character') {
                    shiny::updateTextInput(session, ns(nm), value = NA)
                  } else if (typ %in% c('numeric','integer')) {
                    shiny::updateNumericInput(session, ns(nm), value = NA)
                  } else if (typ == 'Date') {
                    shiny::updateDateInput(session, ns(nm), value = NA)
                  }
